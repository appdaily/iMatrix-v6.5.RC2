...
function updateView(id){
	if(id!=""&&typeof(id)!='undefined'){
		...
		if(ids.length==0){
			...
			alert(iMatrixMessage.selectOneInfo);
		}
	}
}
...
function deleteViews(){
	...
	if(ids==''){
		...
		if(canPost){
			...
			alert(iMatrixMessage["formManager.deleteDraftInfo"]);
		}
	}
}
...
function defaultDisplaySet(type){
	...
	if(ids==''){
		...
		if(confirm(iMatrixMessage["formManager.setOrCancelInfo"])){
			$("#page_id").attr("value",ids[0]);
		...
		if(versionType=="online"){
			if(ids.length==1){
				...
				if(state=='DISABLE' || state=='DRAFT'){
					if(confirm(iMatrixMessage["pageManager.confirmStartInfo"])){
					...
					alert(iMatrixMessage["pageManager.confirmRunFInfo"]);
				}
			}else{
				alert(iMatrixMessage["pageManager.confirmRunInfo"]);
			}
		}else{
			if(confirm(iMatrixMessage["pageManager.confirmStartOrForbidden"])){
				$.each(ids, function(i){
					if($("#pageIds").attr("value")==""){
						$("#pageIds").attr("value",ids[i]);
					}else{
	...
	$("#code").blur(function(){
		$.ajax({
			   ...
			   success: function(data){
			   		if(data=="true"){
			   			alert(iMatrixMessage.code+$("#code").attr("value")+iMatrixMessage.exist);
		   				$("#code").attr("value","");
		   				$("#code").focus();
			   		}
			   }
...
function previewPage(){
	if($("#page_id1").attr("value")!=""){
		...
		alert(iMatrixMessage["pageManager.savePreviewInfo"]);
	}
