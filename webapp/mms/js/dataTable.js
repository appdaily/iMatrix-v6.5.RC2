...
function changeTableInfo(url,callback){
	...
	if(ids==""){
		...
		if(versionType=="online"){
			...
			if(state!= "DRAFT"){
				alert(iMatrixMessage["formManager.updateSheetValidate"]);
				return;
			}
		}
	...
	if(ids==''){
		...
		if(versionType=="online"){
			$.each(ids, function(i){
				...
				if(state!= "DRAFT"){
					alert(iMatrixMessage["formManager.delelteStartForbiddenInfo"]);
					canPost = false;
				}
			});
		}
	...
	if(ids==''){
		...
		if(versionType=="online"){
			$.each(ids, function(i){
				...
				var state=jQuery("#dataTables").jqGrid("getCell",id,"tableState");
				if(state!= "DRAFT"){
					alert(iMatrixMessage["formManager.chooseDraftValidate"]);
					canPost = false;
				}
			});
		}
	...
	$("#inputForm").validate({
		submitHandler: function() {
			if('true'==$("#flowable").val()&&''==$("#workflowCode").val()){
				alert(iMatrixMessage["formManager.workFlowCodeValidate"]);
				return;
			}
