	...
	<script type="text/javascript">
		...
		function checkDeptName(){
			...
			if(code.indexOf(">")>=0||code.indexOf("<")>=0||code.indexOf("\"")>=0||code.indexOf("'")>=0
				...
				alert(iMatrixMessage['department.codeFail'] +" >、<、\"、'、/、 "+iMatrixMessage['department.Spaces']);//编码不能包含符号>、<、\"、'、/、空格
				return;
			}else  if(iMatrix.validateValueTrim(departmentName)){
				alert(iMatrixMessage['department.departmentNameNotContainesSpaces']);//部门名称前后不能包含空格
				return;
			}else  if(iMatrix.validateValueTrim(shortTitle)){
				alert(iMatrixMessage['department.abbreviationNameNotContainesSpaces']);//简称前后不能包含空格
				return;
			}
			$.ajax({
				type : "POST",
				...
				success : function(data){
					if(data!=''){
                       if(data=='departmentCodeFalse'){
                          alert(iMatrixMessage['department.departmentCodeRepeat']+"!");//部门编码重复
                       }else if(data=='departmentNameFalse'){
                    	   alert(iMatrixMessage['department.departmentNameRepeat']+"!");//部门名称重复
                       }else if(data=='branchCodeFalse'){
                    	   alert(iMatrixMessage['department.branchCodeRepeat']+"!");//分支机构编码重复
                       }else if(data=='branchNameFalse'){
                    	   alert(iMatrixMessage['department.branchNameRepeat']+"!");//分支机构名称重复
                       }else if(data=='branchNameFalseOfCompany'){
                    	   alert(iMatrixMessage['department.branchNameAndGroupRepeat']+"!");//分支机构名称不能与集团重复
                       }else if(data=='branchShortNameFalse'){
                    	   alert(iMatrixMessage['department.branchAbbreviationNameRepeat']+"!");//分支机构简称重复
                       }else if(data=="branchCodeFalseOfCompanyCode"){
                           alert(iMatrixMessage['department.branchCodeAndGroupCodeRepeat']);//分支机构编码与集团编码重复
                       }
					}else{
						$('#inputForm').submit();
					}
				...
				error: function(){alert(iMatrixMessage['department.error']);},//错误
				onerror : "",
		...
		function updateDept(){
			...
			if(deptId==''){
				alert(iMatrixMessage["department.selectDepLeft"]);//请在左边树上选择部门
				return;
			}
		...
		function deleteDept(){
			...
			if(childLength>0){
				if("USERSBYDEPARTMENT"==selectNodeType){
					alert(iMatrixMessage["department.deleteChildDepFirst"]);//请先删除子部门或分支机构
				}else{
					alert(iMatrixMessage["department.deleteBranchDep"]);//请先删除分支机构下的部门、人员、工作组、分支机构、分支机构授权管理、角色
				}
				return;
			}
			if("USERSBYDEPARTMENT"==selectNodeType){
		...
		function deptAddUserSubmit(){
			...
			if(typeof(users)=='undefined'||users==''){
                alert(iMatrixMessage['user.seleteUser']+"!");  //请选择用户
                return;
			}
			...
			if(users!=""){
				if(!isAllInSameBranch(departmentSubCompanyId,userSubCompanyIds)){
                     if(confirm(iMatrixMessage['department.branchesAcrossCandidates']+"!")){//您已经跨分支机构选人，如果继续执行原来的分支机构将不存在这些人
                    	 $.ajax( {
	     	    				data : {
	     	    				...
	     	    				success : function(data, textStatus) {
		     	    				if(data!=''){
                                       alert(iMatrixMessage['department.loginNameBelow']+":"+data+iMatrixMessage['department.baranchHasExist']+"!");//如下用户登录名    当前分支机构已存在,已被自动过滤,不能被添加
			     	    			}
					 ...
					 $.ajax( {
  	    				...
  	    				success : function(data, textStatus) {
     	    				if(data!=''){
                               	alert(iMatrixMessage['department.loginNameBelow']+":"+data+iMatrixMessage['department.baranchHasExist']+"!");
	     	    			}
  	    					//var treeSelectedNode = $('#tree_selected_node').val();
  	    					//ajaxSubmit('deptAddUserForm', '${acsCtx}/organization/department!departmentAddUser.action?treeSelectedNode='+treeSelectedNode, 'acs_content,',add_user_call_back);
  	    					departmentAddUser(users);
		...
		function removeUsers(){
			...
			if(node!=''){
               ...
               if(type=='USERSBYBRANCH'){
            	   alert(iMatrixMessage["department.selectDepLefNode"]);//请选择左边树上的部门节点
   				   return;
               }
			}
			...
			if(uIds==''){
				alert(iMatrixMessage["department.selectRemoveUser"]);//请选择要移除的用户
				return;
			}else{
