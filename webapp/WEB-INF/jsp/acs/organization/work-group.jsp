...
<head>
	...
	<script type="text/javascript">
		...
		function updateWorkgroup(){
			...
			if(ids==''){
				alert(iMatrixMessage["common.pleaseSelect"]);//请先选择
			}else if(ids.length > 1){
				 alert(iMatrixMessage["common.pleaseSelectOne"]);//只能选择一条！
			}else{
		...
		function deleteWorkgroup(){     
			...
			if(ids==''){
				...
				if(confirm(iMatrixMessage["workgroup.sureDeleteWorkGroup"])){//确定删除工作组？
					$.ajax({
						data:{ides:ids.join(",")},
						type:"post",
		...
		function submitForm(){
			...
			if(uIds==''){
				alert(iMatrixMessage['workgroup.noSelectedUser']);//没有选中的用户！
				return;
			}else{
				$("#ids").attr("value",uIds);
		...
		function saveWorkgroup(){
			...
			if(code.indexOf("role-")==0&&code.length>27){
				alert(iMatrixMessage["workgroup.defaultCodeLonest27"]);//默认编号时超出最大长度27
				return;
			}
			...
			if(iMatrix.validateValueTrim(workGroupName)){//最前面有空格
				alert(iMatrixMessage["workgroup.workgroupNameNotContainsSpace"]);//工作组名称前后不能包含空格!
				return;
			}
			...
			$.ajax({
				...
				success : function(data){
					if("ok"==data){
						...
						alert(iMatrixMessage["workgroup.workgroupCodeRepeat"]+"\n"+iMatrixMessage["workgroup.workgroupCodeRepeat"]);
					}else if("codeRepeat"==data){
						alert(iMatrixMessage["workgroup.workgroupCodeRepeat"]);//工作组编号重复！
					}else if("nameRepeat"==data){
						alert(iMatrixMessage["workgroup.workgroupCodeRepeat"]);//工作组名称重复！
					}
				},
