...
function selectDepartment(){
	...
	if(node!='undefined'&&node!=''&&(node.indexOf('USERSBYDEPARTMENT')>=0||node.indexOf('USERSBYBRANCH')>=0)){
		...
		$.ajax({
			...
			success:function(data, textStatus){
				if(data=="false"){
					...
					alert(iMatrixMessage["acs.pleaseDeleteThe"]);
				}else{
					alert(iMatrixMessage["acs.cannotSetThe"]);
				}
			},
		...
		if(currentDeptId!=''){//修改
			if(isBranch=="true"){//如果是分支机构
				...
				if(currentDeptSubCompanyId!=''){//分支机构下的部门
	               alert(iMatrixMessage["acs.youHaveCross"]);
	               return;
				}else{//如果是公司下的部门
					var name = node.split(split_two)[1];
					parent.setParentDeptName(name);
		...
		alert(iMatrixMessage["acs.pleaseSelectDepartment"]);
	}
}
