...
<head>
	...
	<script type="text/javascript">
    			...
    			$.ajax( {
    				...
    				success : function(data, textStatus) {
    					if(data=="loginNameFlase"){
    						 alert("<s:text name='user.usernameRepat'/>");
        			    }else{
    						if(!validateName($("#loginName").val())){
    							...
    							alert("<s:text name='user.selectEmailConfig'/>！");//请选择邮箱配置
    					    }else{
        					    if(data=="loginNameMessage"){
            					    if(confirm("<s:text name='user.loginNameHasExist'/>？")){//你的登录名在其他分支机构也存在，确定保存吗
    		    			 	      saveUser(); 
            					    }
        					    }else{
        					    	 saveUser(); 
		...
		function opt(url_, opt, id,id2) {
			...
			var bId = $('#brcheId').val();
			if(uIds==''){
				alert("<s:text name='user.pleaseSelect'/>");//请先选择
			}else if(uIds.length > 1){
				 alert("<s:text name='user.pleaseSelectOne'/>！");//只能选择一条
			}else{
				if(opt!="LOOK"){
					$("#ajaxId").val(uIds);
					$("#edit").val(opt);
		...
		function opt_delete(url_, opt) {
			...
			if(uIds==''){
				...
				if(confirm("<s:text name='user.sureRemove'/>？")){//确认移除吗
					setPageState();
		...
		function importUser(){
			$.colorbox({href:'${acsCtx}/organization/user-showImportUser.action',
				iframe:true, innerWidth:350, innerHeight:100,overlayClose:false,title:"<s:text name='user.importUser'/>"});//导入用户
		} 
		 ...
		 */
	  	function modifyPassWord(id,url_) {
		  	if(versionType=="online"){
		  		...
		  		$.colorbox({href:url_+'?id='+id,iframe:true, innerWidth:640, innerHeight:150,overlayClose:false,title:"<s:text name='userInfo.updatePassword'/>"});//修改密码
		  	}
		...
		function unlockUser(){
			...
			if(uIds==''){
				...
				if(confirm("<s:text name='user.confirmUnlock'/>？")){//确认解锁吗
					$.ajax({
						   type: "POST",
			...
			if(uIds==''){
				...
				if(confirm("<s:text name='user.confirmUnlock'/>？")){//确认解锁吗
					$.ajax({
						   ...
						   success: function(data, textStatus){
					                alert("<s:text name='user.unlockDatasuccess'/>");//数据解锁成功
					                return;
						},error : function(XMLHttpRequest, textStatus) {
								alert(textStatus);
						}
		...
		function workgroupAddUserSubmit(){
			...
			}else alert('<s:text name="user.selectedUserHasBeen"/>');//所选用户已在该部门
		}
		...
		*/
		function clearUser(){
			...
			if(uIds==''){
				...
				if(confirm("<s:text name='user.sureDelete'/>？")){
					$.ajax({
						   type: "POST",
		...
		function userTree(){
			...
			var uIds = jQuery("#main_table").getGridParam('selarrrow');
			if(uIds.length==0){
				alert("<s:text name='user.selectPerson'/>!");//请选择人员
				return;
			}
			var zTreeSetting={
					leaf: {
		...
		function copyRoleToUser(sourceIds){
			...
			if(!userIds||userIds==""){
				alert("<s:text name='user.selectNeedAuthorizedPer'/>！");//请选择需要被授权的人员
				return;
			}
			if(confirm("<s:text name='user.copyAuthorizes'/>")){//确认复制？(管理员权限不会被复制)
				$.ajax({
		...
		function copyRoleToDepartment(sourceIds){
			...
			if(!deptIds||deptIds==""){
				alert("<s:text name='user.selectNeedAhthorizedDep'/>！");//请选择需要被授权的部门
				return;
			}
			if(confirm("<s:text name='user.copyAuthorizes'/>")){//确认复制？(管理员权限不会被复制)
				$.ajax({
		...
		function copyRoleToWorkgroup(sourceIds){
			...
			if(!workgroupIds||workgroupIds==""){
				alert("<s:text name='user.selectNeedAhthorizedWorkg'/>！");//请选择需要被授权的工作组
				return;
			}
			if(confirm("<s:text name='user.copyAuthorizes'/>")){//确认复制？(管理员权限不会被复制)
				$.ajax({
