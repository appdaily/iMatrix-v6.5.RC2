...
<head>
	...
	<script type="text/javascript">
		...
		function createDataRule(url){
			...
			if($("#menuId").val()==''){
				alert(iMatrixMessage['permission.selectCorrespondingMenu']);//"请选择对应的菜单！");
			}else{
					...
					if(cansave){
						...
						if(cansave){
							...
							if(leftBracketCount!=rightBracketCount){
								iMatrix.addBtn();
								alert(iMatrixMessage['dataAuth.parenthesisDoesNotMatch']);////"‘左括号’与‘右括号’不匹配！");
							}else{
		...
		function codeRule(){
			...
			if(code.indexOf("dataRule-")==0&&code.length>27){
				alert(iMatrixMessage['permission.defaultCodeLonest27']);//"默认编号时超出最大长度27");
				return false;
			}
			if(code.indexOf(" ")==0||code.lastIndexOf(" ")==code.length-1){//最前面有空格
				alert(iMatrixMessage['permission.codeNotContainsSpace']);//"编码前后不能包含空格!");
				return false;
			}
			if(code.indexOf(">")>=0||code.indexOf("<")>=0||code.indexOf("\"")>=0||code.indexOf("'")>=0
					||code.indexOf("/")>=0){//包含特殊字符
				alert(iMatrixMessage['department.codeFail'] + ">、<、\"、'、/");///"编码不能包含符号>、<、\"、'、/");
				return false;
			}
			return true;
		}
		...
		function selectDataTable(pos){
			...
			if(dataTableName != ''&&settingval=="false"){//当修改数据表且是高级设置时
				if(confirm(iMatrixMessage["permission.sureUpdateDatatables"])){//确定修改数据表吗？所选择的规则条件将被删除！
		...
		function fieldNameClick(obj){
			...
			if($("#dataTableId").val()!=''){
				...
				alert(iMatrixMessage["permission.pleaseSelectform"]);//"请先选择表单！");
			}
		}
		...
		function validateFieldString(obj,conditionId){
			...
			if(value.length>20){
				...
				alert(iMatrixMessage['permission.contionNotLonest20']);////"条件长度不能大于20！");
				$(obj).attr("value", val);
			}else{
				var b;
				if((b=value.indexOf("{"))>=0 || (b=value.indexOf("}"))>=0 || (b=value.indexOf("\'"))>=0 || (b=value.indexOf("\""))>=0){
					var val = value.substring(0, b);
					alert(iMatrixMessage['permission.contionFail']);//"条件不能包含空格、单引号、双引号和大括号");
					$(obj).attr("value", val);
				}
			}
		}
...
// 				alert("为确保系统的正常演示,不能删除编号为“PRODUCT_RULE1”和“PRODUCT_RULE2”的数据！");
				alert(iMatrixMessage['permission.cannotDeleteCode']+"“PRODUCT_RULE1”"+iMatrixMessage['permission.and']+"“PRODUCT_RULE2”"+iMatrixMessage['permission.data']);
			}
		}
