			...
			if("entering"==$("#pageSign").val() || ""==$("#pageSign").val()){
				if("valuesSign"==$("#valuesSign").val()){
					...
					if(fieldValue.length<=0){
						alert(iMatrixMessage["acs.data.rule.pleaseSelectAConditionValue"]);//请选择一个条件值！
						return;
					}else{
					...
					if(conditionNames != ""){
						...
						alert(iMatrixMessage["acs.data.rule.pleaseEnterAConditionValue"]);//请输入条件值！
						return;
					}
				}
				...
				if(fieldNames.length<=0){
					alert(iMatrixMessage["acs.data.rule.pleaseSelectAStandardValue"]);//请选择一个标准值！
					return;
				}else{
				...
				if(conditionNames != ""){
					...
					alert(iMatrixMessage["acs.data.rule.pleaseFillInBeanName"]);//请填写beanName！
					return;
				}
			}
