...
<head>
	...
	function createPermission(){
		...
		if(dataRuleId!='' && typeof (dataRuleId)!='undefined'){
			...
			alert(iMatrixMessage["dataAuth.selectDataClassification"]);//请选择对应的数据分类!
		}
	}
		...
		$("#viewSaveForm").validate({
			submitHandler: function() {
				...
				if(checkedAuths.length>0){
					...
					iMatrix.addBtn();
					alert(iMatrixMessage['dataAuth.selectDataAuth']);//"请选择操作权限"
				}
			},
			rules: {
				code:"required",
	...
	function updatePermission(){
		...
		if(boxes.length<=0){
			...
			alert(iMatrixMessage["dataAuth.onlySelectOneData"]);//只能选择一条记录
			return;
		}else{
	...
	function deletePermission(){
		var boxes = jQuery("#page").jqGrid("getGridParam",'selarrrow');
		if(boxes.length<=0){
			alert(iMatrixMessage["selectInfo"]);///"请选择记录"
			return;
		}else{
	...
	function conditionNameClick(obj){
		...
		if(itemType==""){
			alert(iMatrixMessage["dataAuth.selectContionType"]);return;//先选择条件类型！
		}
	...
	function getRoleInformation(obj){
		...
		if(ids!=""&&ids.length>0){
			...
			alert(iMatrixMessage['authorization.pleaseSelectRole']);////"请选择角色");
		}
	}
	...
	function  validatePermissionSave(){
		if($("#code").attr("value")==""){
			...
			if(existable){//当焦点在编号文本框时,点击保存会走2次isPermissionCodeExist方法,为了预防该种情况设置了该条件
				alert(iMatrixMessage["dataAuth.codeIsExist"]);//该授权编号已存在
			}else{
