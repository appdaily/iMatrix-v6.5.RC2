...
<head>
	...
	function createPermission(){
		...
		if(menuId!='' && typeof (menuId)!='undefined'){
			...
			alert(iMatrixMessage["dataAuth.selectSystem"]);
		}
	}
	function validatePermission(){
		...
		$("#viewSaveForm").validate({
			submitHandler: function() {
				...
				if(boxes<=0){
					iMatrix.addBtn();
					alert(iMatrixMessage["dataAuth.selectPerson"]);//请选择人员
				}else{
	...
	function updatePermission(){
		...
		if(boxes.length<=0){
			alert(iMatrixMessage['dataAuth.selectDatas']);//"请选择记录"
			return;
	...
	function deletePermission(){
		...
		if(boxes.length<=0){
			...
			if(confirm(iMatrixMessage["log.sureDelete"])){
