	...
	<script type="text/javascript">
		...
		function importRole(){
			$.colorbox({href:'${acsCtx}/authorization/role-showImportRole.action',
				iframe:true, innerWidth:350, innerHeight:100,overlayClose:false,title:iMatrixMessage["authorization.importRole"]});
		}
		function accredit(){
			$.colorbox({href:'${acsCtx}/authorization/role-showAccredit.action',
				iframe:true, innerWidth:350, innerHeight:100,overlayClose:false,title:iMatrixMessage["authorization.importRoleUserContion"]});
		}
		//导入用户角色关系
		function createRole(){
			if(''==$("#_systemId").val()){
				alert(iMatrixMessage["authorization.selectSystem"]);//"请选择系统！"
			}else{
				//隐藏查询框
				HideSearchBox();
				...
				$.ajax({
					...
					success:function(data, textStatus){
						if("ok"==data){
							...
							alert(iMatrixMessage['authorization.noAuthUpdateRole']);//"无权限修改此角色！"
						}
					},
		...
		function deleteRole(){
			...
			if(roles.length > 0){
				if(deletable(roles)){
					if(confirm(iMatrixMessage['authorization.sureDelete'])){//"确定要删除吗？"
			...
			$("#roleForm").validate({
				...
				if(validateRolename($("#roleName").val())){
					if("branchAdmin"==$("#adminSign").val() && $("#branchSum").val()>1 && $("#subCompanyId").val()==''){
						alert(iMatrixMessage['authorization.selectBranchs']);//请选择所属分支机构！
					}else{
						var code = $("#roleCode").val();
						if(code.indexOf("role-")==0&&code.length>23){
							alert(iMatrixMessage['authorization.defaultCodeLonest23']);//"默认编号时超出最大长度23"
							return;
						}
						if(code.indexOf(" ")==0||code.lastIndexOf(" ")==code.length-1){//最前面有空格
							alert(iMatrixMessage["workgroup.codeNotContainsSpace"]);//"编码前后不能包含空格!"
							return;
						}
						if(code.indexOf(">")>=0||code.indexOf("<")>=0||code.indexOf("\"")>=0||code.indexOf("'")>=0
								||code.indexOf("/")>=0){//包含特殊字符
							alert(iMatrixMessage['workgroup.codeNotContainsfuhao']+">、<、\"、'、/");//编码不能包含符号
							return;
						}
						$.ajax({
							...
							success:function(data, textStatus){
								if("ok"==data){
									...
									alert(iMatrixMessage['authorization.roleCodeRepeat'] + "\n" + iMatrixMessage['authorization.roleNameRepeat']);//角色编号重复！\n角色名称重复！
								}else if("codeRepeat"==data){
									alert(iMatrixMessage['authorization.roleCodeRepeat']);//角色编号重复！
								}else if("nameRepeat"==data){
									alert(iMatrixMessage['authorization.roleNameRepeat']);//"角色名称重复！"
								}
							},
							complete:function(XMLHttpRequest, textStatus){},
					        error:function(){
		...
		function selectSystem(){
			...
			if(!canCopy){
				alert(iMatrixMessage['authorization.cannotCopyAuth']);//不能复制安全管理员、系统管理员、审计管理员、分支机构管理员的权限
				return;
			}
			if(!canCopyWf){
				alert(iMatrixMessage['authorization.cannotCopyAuthManager']);//不能复制工作流管理员、流程定义管理员的权限
				return;
			}
			var roleIds="";
			if(roles.length >= 1){
		...
		function closeFun(){
			...
			if(roleIds=="system"){
				alert(iMatrixMessage['authorization.pleaseSelectRole']);
				return;
			}
			if(confirm(iMatrixMessage['authorization.sureCopyAuth'])){//"确定复制权限吗?"
				if(roleIds!=""&&roleIds.length>0){
					...
					$.ajax({
						...
						beforeSend:function(XMLHttpRequest){},
						success:function(data, textStatus){
							alert(iMatrixMessage['authorization.sureCopyAuthSuccess']);//复制权限成功
						},
						complete:function(XMLHttpRequest, textStatus){},
				        error:function(){
