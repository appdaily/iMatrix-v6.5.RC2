	...
	<script type="text/javascript">
		...
		function opt(operate){
			...
			if(operate != "REMOVE_ALL"){
				...
				if(confirm(iMatrixMessage["authorization.sureDeleteSelectedItems"])){//确定要从角色中移除所选项?
					$("#removeForm").submit();
					return;
				}else{ return; }
			}
		...
		function validateRemove(){
			...
			for(var i=0; i<userList.length; i++){
				...
				if((name=="systemAdmin"&&roleName=="系统管理员")||(name=="securityAdmin"&&roleName=="安全管理员")||(name=="auditAdmin"&&roleName=="审计管理员")){ 
					...
					alert(iMatrixMessage["authorization.defaultManagerCannotDelete"]);//系统默认的管理员不能删除！
					return true;
				}
				}
			}
			...
			alert(iMatrixMessage['authorization.selectDelete']);//请选择需要移除的项
			return true;
		}
