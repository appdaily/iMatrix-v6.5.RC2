	...
	<script type="text/javascript">
		...
		function updateFormStyle(){
			...
			if(ids==""){
				...
				showMessage("message", "<font color=\"red\">"+iMatrixMessage.selectOneOnlyInfo+"</font>");
				return;
			}
				...
				submitHandler: function() {
				   var code = $("#code").val();
					if(validateFormCode(code)){
						...
						alert(iMatrixMessage.formCodeIncludeInfo);
					}
				},
				rules: {
					code:"required",
		...
		function updateVersion(){
			if(versionType=="online"){
				alert(iMatrixMessage.onlineVesionInfo);
			}else{
				$("html").attr("value",editor.html());
				$("#inputForm").ajaxSubmit(function (data){
			...
			$("#code").blur(function(){
				$.ajax({
					   ...
					   success: function(data){
					   		if(data=="true"){
					   			alert(iMatrixMessage.thisCode+$("#code").attr("value")+iMatrixMessage.exist);
				   				$("#code").attr("value","");
				   				$("#code").focus();
					   		}
					   }
			...
			if(ids==""){
				...
				if(versionType=="online"){
					...
					if(state=='ENABLE'||state=='DISABLE'){
						alert(iMatrixMessage.shieldStartOrStopInfo);
					}else{
						$.each(ids, function(i){
		...
		function exportFormView(){
			...
			var ids = jQuery("#page").getGridParam('selarrrow');
			if(ids==''){
				if(confirm(iMatrixMessage.exportSystemAllFormInfo)){
		...
		function importFormView(){
			$.colorbox({href:'${mmsCtx}/form/show-import-form-view.htm',
				iframe:true, innerWidth:350, innerHeight:100,overlayClose:false,title:iMatrixMessage["formManager.importFormViewColorboxTitle"]});
		}
		...
		function deleteCompleteForm(){
			...
			if(ids==''){
				...
				if(confirm(iMatrixMessage.deleteCompleteFormInfo)){
					$.each(ids, function(i){
