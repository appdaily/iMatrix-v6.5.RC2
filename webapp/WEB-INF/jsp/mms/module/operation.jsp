		...
		if(code!=""){
			...
			for(var i=0;i<rows.length;i++){
				if(rows[i].id!=operationId){
					if(rows[i].code==code){
						...
						alert(iMatrixMessage.thisCode+code+iMatrixMessage.exist);
						return;
					}
				}
			}
			...
			if(!isExist){
				...
				if(!isParent){
					...
					isExist=true;alert(iMatrixMessage.thisCode+code+iMatrixMessage.exist);return;}
				}
				//如果不存在
				if(!isExist){
					...
					$.ajax({
						...
						error : function() {
							alert(iMatrixMessage.serverInfo);
						}
					});
				}
			}
