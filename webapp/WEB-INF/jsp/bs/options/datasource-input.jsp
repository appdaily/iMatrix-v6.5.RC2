	...
	function testDatasource(){
		...
		if("other.jdbc.Driver"==$("#driveName").attr("value")){
			if($("#otherDriveName").attr("value")==""){
				alert(iMatrixMessage["interfaceManager.dataSourceDriverInfo"]);
				return;
			}
		...
		$.ajax({
			   ...
			   success: function(msg){
				   if(msg=="true"){
					  alert(iMatrixMessage["interfaceManager.testSuccess"]);
				   }else{
					  var errorMsg = msg.substring(msg.indexOf(";")+1);
					  alert(iMatrixMessage["interfaceManager.testFail"]+errorMsg);
				   }
			   }
		});
	}
