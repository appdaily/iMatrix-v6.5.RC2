	...
	<script type="text/javascript">
		...
		function validateSelected(selectId){
			...
			if(count > 1){
				...
				alert(iMatrixMessage["basicSetting.onlyOneOptionInfo"]);
			}else{
				selectFlag = true;
			}
		}
		...
		function deleteOptionRow(obj,row,optionId){
			if($("#optionTable tr").length<=2){
				alert(iMatrixMessage["basicSetting.needOptionInfo"]);
				return;
			}
			if(confirm(iMatrixMessage.deleteInfo)){
				//deleteValidate(row);
			...
			for(var i = 0; i<inputs.length; i++){
				for(var j = i+1; j<inputs.length; j++){
					if($(inputs[i]).val()==""||$(inputs[i]).val()==null){
						alert(iMatrixMessage["basicSetting.optionNameNotNull"]);
						return false;
					}
					if($(inputs[i]).val() == $(inputs[j]).val()){
						...
						alert(iMatrixMessage["basicSetting.showSort"]+" "+optIndexI+" "+iMatrixMessage["basicSetting.optiongroupAnd"]+" "+optIndexJ+" "+iMatrixMessage["basicSetting.optiongroupRepeat"]);
						return false;
					}
				}
			}
			...
			for(var i = 0; i<inputsValue.length; i++){
				for(var j = i+1; j<inputsValue.length; j++){
					if($(inputsValue[i]).val()==""||$(inputsValue[i]).val()==null){
						alert(iMatrixMessage["basicSetting.optionValueNotNull"]);
						return false;
					}
				}
			}
