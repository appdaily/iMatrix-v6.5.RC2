		...
		$("#jobInfoFrom").validate({
			submitHandler: function() {
				...
				if(emails.length>1000){
					alert(iMatrixMessage["bs.jobInfo.fieldEmailReminder"]+emails.length);
					return;
				}
				var rtxAccounts = $("#rtxAccounts").attr("value");
				if(rtxAccounts.length>1000){
					alert(iMatrixMessage["interfaceManager.rtxValidate"]+rtxAccounts.length);
					return;
				}
				var phoneReminderNums = $("#phoneReminderNums").attr("value");
				if(phoneReminderNums.length>1000){
					alert(iMatrixMessage["interfaceManager.messageValidate"]+phoneReminderNums.length);
					return;
				}
				var officeHelperReminderNames = $("#officeHelperReminderNames").attr("value");
				if(officeHelperReminderNames.length>490){
					alert(iMatrixMessage["interfaceManager.officerValidate"]+officeHelperReminderNames.length);
					return;
				}
	...
	function isInterfaceExist(){
		$.ajax({
			   ...
			   success: function(msg){
				   if(msg=="true"){
					  alert(iMatrixMessage["interfaceManager.sourceCodeValidate"]);
				   }else{
					   $("#jobInfoFrom").submit();
				   }
			   }
