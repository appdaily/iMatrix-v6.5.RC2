function add()
	{
	  ...
	  var dataSrc = $('#dataSrc').attr("value");
		if(field=="")
		{
			alert(iMatrixMessage["formManager.databaseFieldValidate"]);
			return;
		}	
		var control=$('#itemTitle').attr("value");
		if(control=="")
		{
			alert(iMatrixMessage["formManager.mapControlNameValidate"]);
			return;
		}
		...
		for(var i=1;i<rows.length;i++)
	  	{
			  ...
			  if(dataField==field)
			  {
				  alert(iMatrixMessage["formManager.addValidate"]);
				  return;
			  }
		}
	...
	function del(obj)
	{
	 ...
	 function getListControlData(){
		 var dataSrc=$('#dataSrc').attr("value");
		 if(dataSrc==0){
			 alert(iMatrixMessage["formManager.chooseDateSource"]);
			 return;
		 }
