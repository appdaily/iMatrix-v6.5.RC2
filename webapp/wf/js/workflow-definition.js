function doQuery(obj){
	...
	function changeTransactor(btnId){
		...
		if(ids==''){
			...
			if(state=="SUBMIT"){
				...
				alert(iMatrixMessage["wf.engine.processHasChange"]);
			}
		}
	}
	//环节跳转/弹窗“选择环节”
	...
	function delete_monitor_workflow(position){
		...
		if(ids==''){
			...
			if(confirm(iMatrixMessage["wf.engine.areYouSure"])){
	...
	function end_workflow_def(){
		...
		if(ids==''){
			...
			if(confirm(iMatrixMessage["wf.engine.doYouConfirm"])){
				 var prmt = '';
                for(var i=0;i<ids.length;i++){
                    if(prmt != '') prmt += '&';
                    prmt+=('workflowIds='+ids[i]);
	...
	function compel_end_workflow_def(){
		...
		if(ids==''){
			...
			if(confirm(iMatrixMessage["wf.engine.confirmTheMandatory"])){
				var prmt = '';
				for(var i=0;i<ids.length;i++){
					if(prmt != '') prmt += '&';
					prmt+=('workflowIds='+ids[i]);
			...
			$.ajax({
				   ...
				   success: function(systemUrl){
					if(systemUrl==""||typeof(systemUrl)=='undefined'){
						alert(iMatrixMessage["wf.engine.pathIsNotCorrect"]);
					}else{
						var screenWidth=screen.availWidth-12;
						var screenHeight=screen.availHeight-58;
...
//	}
	...
	function urgen_done(){
		...
		if(ids==''){
			...
			if(url==""||typeof (url)=="undefined"){
				alert(iMatrixMessage["wf.engine.thereIsNo"]);
			}else{
				var screenWidth=screen.availWidth-12;
				var screenHeight=screen.availHeight-58;
	...
	function addTransactor(){
		...
		if(ids==''){
			...
			if(state=="SUBMIT"){
				...
				alert(iMatrixMessage["wf.engine.theProcessAdd"]);
			}
		}
	}
	...
	function delTransactor(btnId){
		...
		if(ids==''){
			...
			if(state=="SUBMIT"){
				...
				alert(iMatrixMessage["wf.engine.theProcessReduced"]);
			}
		}
	}
	...
	function pause_workflow(wfdId){
		...
		if(item.length==0){
			...
			if(confirm(iMatrixMessage["wf.engine.confirmTheSuspension"])){
				var prmt = '';
	...
	function continue_workflow(wfdId){
		...
		if(item.length==0){
			...
			if(confirm(iMatrixMessage["wf.engine.confirmToContinue"])){
				var prmt = '';
                for(var i=0;i<item.length;i++){
                    if(prmt != '') prmt += '&';
                    prmt+=('workflowIds='+item[i]);
