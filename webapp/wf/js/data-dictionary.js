...
function deleteDictType(url){
	...
	if(ids==''){
		...
		if(confirm(iMatrixMessage["wf.engine.referenceToThe"])){
			$("#dictIds").attr("value",ids);	
			setPageState();
...
function addUsers(id){
	if($('#userNamesView').html()==iMatrixMessage["common.allPersonnel"]){
		alert(iMatrixMessage["wf.engine.hasBeenAll"]);
	}else{
...
function clearAway(){
	if($('#slcMan').children().length==0){
		alert(iMatrixMessage["wf.engine.noManager"]);
	}else{
...
function validateTypeNo(){
	$.ajax({
		   ...
		   success: function(data){
				if(data=="true"){
					alert(iMatrixMessage["wf.engine.numberExists"]);
					$("#no").attr("value","");
				}
		   }
		}); 
